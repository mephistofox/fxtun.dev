...
const faqItems = computed(() => tm('compare.cloudpub.faq') as FaqItem[])
useFaqSchema(faqItems.value.map(item => ({ question: item.q, answer: item.a })), '-cloudpub')
...
<template>
  <ComparePageLayout competitor-name="CloudPub" competitor-slug="cloudpub">
    <!-- Comparison Table -->
    <template #table>
            ...
            <table class="compare-table w-full">
              ...
              <tbody>
                <tr v-for="row in tableRows" :key="row">
                  <td class="font-medium">{{ t(`compare.cloudpub.table.${row}.label`) }}</td>
                  <td class="text-center compare-highlight">{{ t(`compare.cloudpub.table.${row}.fxtunnel`) }}</td>
                  <td class="text-center">{{ t(`compare.cloudpub.table.${row}.competitor`) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
