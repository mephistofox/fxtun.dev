...
const faqItems = computed(() => tm('compare.cloudflare.faq') as FaqItem[])
useFaqSchema(faqItems.value.map(item => ({ question: item.q, answer: item.a })), '-cloudflare')
...
<template>
  <ComparePageLayout competitor-name="Cloudflare Tunnel" competitor-slug="cloudflare">
    <!-- Comparison Table -->
    <template #table>
            ...
            <table class="compare-table w-full">
              <thead>
                <tr>
                  ...
                  <th class="text-center">Cloudflare Tunnel</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in tableRows" :key="row">
                  <td class="font-medium">{{ t(`compare.cloudflare.table.${row}.label`) }}</td>
                  <td class="text-center compare-highlight">{{ t(`compare.cloudflare.table.${row}.fxtunnel`) }}</td>
                  <td class="text-center">{{ t(`compare.cloudflare.table.${row}.competitor`) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="container mx-auto px-4 max-w-4xl">
          <div v-for="key in sectionKeys" :key="key" class="compare-detail-section">
            <h2 class="text-2xl font-display font-semibold mb-4">
              {{ t(`compare.cloudflare.sections.${key}Title`) }}
            </h2>
            <p class="text-muted-foreground leading-relaxed">
              {{ t(`compare.cloudflare.sections.${key}Text`) }}
            </p>
          </div>
          ...
          <div class="compare-verdict mt-12 border-l-4 border-primary rounded-r-lg bg-primary/5 p-6">
            <h2 class="text-2xl font-display font-semibold mb-4">
              {{ t('compare.cloudflare.sections.verdictTitle') }}
            </h2>
            <p class="text-muted-foreground leading-relaxed">
              {{ t('compare.cloudflare.sections.verdictText') }}
            </p>
          </div>
        </div>
      </section>
