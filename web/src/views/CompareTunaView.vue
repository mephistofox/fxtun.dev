...
const sectionKeys = [
  'pricing', 'protocols', 'limits', 'platform', 'gui', 'selfHosted',
] as const
</script>

<template>
  <ComparePageLayout competitor-name="tuna.am" competitor-slug="tuna">
    <!-- Comparison Table -->
    <template #table>
            ...
            <table class="compare-table w-full">
              ...
              <tbody>
                <tr v-for="row in tableRows" :key="row">
                  <td class="font-medium">{{ t(`compare.tuna.table.${row}.label`) }}</td>
                  <td class="text-center compare-highlight">{{ t(`compare.tuna.table.${row}.fxtunnel`) }}</td>
                  <td class="text-center">{{ t(`compare.tuna.table.${row}.competitor`) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="container mx-auto px-4 max-w-4xl">
          <div v-for="key in sectionKeys" :key="key" class="compare-detail-section">
            <h2 class="text-2xl font-display font-semibold mb-4">
              {{ t(`compare.tuna.sections.${key}Title`) }}
            </h2>
            <p class="text-muted-foreground leading-relaxed">
              {{ t(`compare.tuna.sections.${key}Text`) }}
            </p>
          </div>
          ...
          <div class="compare-verdict mt-12 border-l-4 border-primary rounded-r-lg bg-primary/5 p-6">
            <h2 class="text-2xl font-display font-semibold mb-4">
              {{ t('compare.tuna.sections.verdictTitle') }}
            </h2>
            <p class="text-muted-foreground leading-relaxed">
              {{ t('compare.tuna.sections.verdictText') }}
            </p>
          </div>
        </div>
      </section>
