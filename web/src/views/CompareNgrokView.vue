...
const sectionKeys = [
  'pricing', 'subdomains', 'limits', 'protocols', 'gui', 'inspector',
] as const
</script>

<template>
  <ComparePageLayout competitor-name="ngrok" competitor-slug="ngrok">
    <!-- Comparison Table -->
    <template #table>
            ...
            <table class="compare-table w-full">
              <thead>
                <tr>
                  ...
                  <th class="text-center">ngrok</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in tableRows" :key="row">
                  <td class="font-medium">{{ t(`compare.ngrok.table.${row}.label`) }}</td>
                  <td class="text-center compare-highlight">{{ t(`compare.ngrok.table.${row}.fxtunnel`) }}</td>
                  <td class="text-center">{{ t(`compare.ngrok.table.${row}.competitor`) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="container mx-auto px-4 max-w-4xl">
          <div v-for="key in sectionKeys" :key="key" class="compare-detail-section">
            <h2 class="text-2xl font-display font-semibold mb-4">
              {{ t(`compare.ngrok.sections.${key}Title`) }}
            </h2>
