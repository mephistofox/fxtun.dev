  ...
  <div class="min-h-screen flex items-center justify-center bg-background p-4">
    ...
    <Card class="w-full max-w-md p-6">
      <div class="text-center mb-6">
        ...
        <p class="text-muted-foreground mt-1">{{ t('auth.signUpTitle') }}</p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <Button type="submit" class="w-full" :loading="authStore.loading">{{ t('auth.signUp') }}</Button>
      </form>

      <p class="text-center text-sm text-muted-foreground mt-4">
        {{ t('auth.hasAccount') }}
        <RouterLink to="/login" class="text-primary hover:underline">{{ t('auth.signIn') }}</RouterLink>
      </p>
    </Card>
  </div>
</template>
