        ...
        >
          ...
          >
            ...
            <div class="relative group">
              <div class="bg-code rounded-xl p-4 font-mono text-sm border border-border flex items-center justify-between gap-3 overflow-hidden">
                ...
                >
                  <svg aria-hidden="true" v-if="!copied" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184" />
                  </svg>
                  <svg aria-hidden="true" v-else class="h-4 w-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="inline-flex p-1.5 rounded-2xl bg-surface border border-border">
              ...
              >
                <span class="flex items-center gap-2">
                  <!-- Linux -->
                  <svg aria-hidden="true" v-if="tab.key === 'linux'" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  <!-- macOS -->
                  <svg aria-hidden="true" v-else-if="tab.key === 'macos'" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                  </svg>
                  <!-- Windows -->
                  <svg aria-hidden="true" v-else-if="tab.key === 'windows'" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
          ...
          >
            <div v-if="loading" class="flex justify-center py-8">
              <svg aria-hidden="true" class="h-5 w-5 animate-spin text-muted-foreground" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div v-else-if="filteredDownloads.length > 0" class="flex flex-wrap justify-center gap-4">
              ...
              >
                <svg aria-hidden="true" class="h-5 w-5 text-muted-foreground group-hover:text-primary transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
