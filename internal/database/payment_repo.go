...
func (r *PaymentRepository) Create(p *Payment) error {
	result, err := r.db.Exec(`
		INSERT INTO payments (user_id, subscription_id, invoice_id, amount, status, is_recurring, yookassa_data, provider, provider_data)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		p.UserID, p.SubscriptionID, p.InvoiceID, p.Amount, p.Status, p.IsRecurring, p.YooKassaData,
		p.Provider, p.ProviderData)
	if err != nil {
...
func (r *PaymentRepository) GetByID(id int64) (*Payment, error) {
	p := &Payment{}
	err := r.db.QueryRow(`
		SELECT id, user_id, subscription_id, invoice_id, amount, status, is_recurring, yookassa_data,
		       provider, provider_data, created_at
		FROM payments WHERE id = ?`, id).Scan(
		&p.ID, &p.UserID, &p.SubscriptionID, &p.InvoiceID, &p.Amount, &p.Status,
		&p.IsRecurring, &p.YooKassaData, &p.Provider, &p.ProviderData, &p.CreatedAt)
...
func (r *PaymentRepository) Update(p *Payment) error {
	_, err := r.db.Exec(`
		UPDATE payments
		SET subscription_id = ?, status = ?, yookassa_data = ?, provider = ?, provider_data = ?
		WHERE id = ?`,
		p.SubscriptionID, p.Status, p.YooKassaData, p.Provider, p.ProviderData, p.ID)
	if err != nil {
